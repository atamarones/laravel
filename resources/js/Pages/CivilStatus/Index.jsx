...
export default function Index({ auth, civilStatus, can }) {
    const [processing, setProcessing] = useState(false);

    const handleDelete = (id) => {
        if (confirm('¿Está seguro de que desea eliminar este estado civil?')) {
            setProcessing(true);
            router.delete(route('civil-status.destroy', id), {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Estados Civiles</h2>}
        >
                        ...
                        <div className="p-6 text-gray-900">
                            <div className="sm:flex sm:items-center sm:justify-between mb-6">
                                <div className="w-full max-w-lg">
                                    <SearchInput 
                                        route={route('civil-status.index')}
                                ...
                                {can.create && (
                                    <div className="mt-4 sm:mt-0">
                                        <Link
                                            href={route('civil-status.create')}
                                                ...
                                                {civilStatus.data.map((status) => (
                                                    <tr key={status.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                                            <Link
                                                                href={route('civil-status.show', status.id)}
                                                            ...
                                                            {can.edit && (
                                                                <Link
                                                                    href={route('civil-status.edit', status.id)}
